...
class VisualNavigationSafetyTests(unittest.TestCase):
    ...
    def test_navigation_selector_route_refreshes_and_unwinds_to_home(self) -> None:
        ...
        failed_app = _FakeApplication(failed_runtime)
        with TemporaryDirectory() as directory, patch(
            "tools.validate_visual_navigation.build_application_runner",
    ...
    def test_probe_releases_reservation_and_runtime_when_setup_fails(self) -> None:
        """The outer account lease and connected bundle close on a failed probe."""
        ...
        self.assertEqual(("testing",), app.reserved_accounts)
        self.assertEqual(1, app.reservation.close_count)
        self.assertEqual(1, app.bundle.close_count)
        self.assertEqual(
            ["reservation_enter", "bundle_enter", "bundle_close", "reservation_close"],
...
class _FakeReservation:
    """Tracks the outer account lease used by the probe lifecycle."""
    ...
    def __enter__(self):
        self._events.append("reservation_enter")
        return self
    ...
    def close(self) -> None:
        if self.close_count == 0:
            self.close_count += 1
            self._events.append("reservation_close")


class _FakeRuntimeBundle:
    """Context-managed connected runtime returned by the script runner fake."""

    def __init__(self, runtime: "_FakeRuntime", events: list[str]) -> None:
        self.runtime = runtime
        self._events = events
        self.close_count = 0

    def __enter__(self):
        self._events.append("bundle_enter")
        return self
    ...
    def close(self) -> None:
        if self.close_count == 0:
            self.close_count += 1
            self._events.append("bundle_close")


class _FakeApplication:
    """Application runner fake with the same resource scopes as production."""

    def __init__(self, runtime: "_FakeRuntime") -> None:
        self.runtime = runtime
        self.lifecycle_events: list[str] = []
        self.reserved_accounts: tuple[str, ...] = ()
        self.reservation = _FakeReservation(self.lifecycle_events)
        self.bundle = _FakeRuntimeBundle(runtime, self.lifecycle_events)
        account = SimpleNamespace(require_live_role=lambda role: None)
        self.script_runner = SimpleNamespace(
            config=SimpleNamespace(require_account=lambda account_id: account),
            observation_builder=SimpleNamespace(selector_registry=object()),
            logger=SimpleNamespace(info=lambda *args, **kwargs: None),
            build_connected_runtime_bundle=self.build_connected_runtime_bundle,
        )

    def reserve_accounts(self, account_ids: tuple[str, ...]) -> _FakeReservation:
        self.reserved_accounts = tuple(account_ids)
        self.lifecycle_events.append("reservation_enter")
        return self.reservation

    def build_connected_runtime_bundle(self, *, account, required_role=None) -> _FakeRuntimeBundle:
        del account, required_role
        self.bundle = _FakeRuntimeBundle(self.runtime, self.lifecycle_events)
        return self.bundle
