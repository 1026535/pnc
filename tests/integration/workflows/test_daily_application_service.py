...
class DailyApplicationServiceTests(unittest.TestCase):
    ...
    def test_instances_overlap_but_castles_on_one_instance_do_not(self) -> None:
        ...
        factory = _RecordingFactory(require_overlap=True)
        ...
        self.assertTrue(factory.overlapped.is_set())
...
class _RecordingRunner:
    ...
    def run_castle(self, *, target, boundary) -> DailyCastleRunSummary:
        ...
        # A rendezvous proves concurrency without assuming thread scheduling fits
        # into a 20 ms sleep, including under coverage instrumentation.
        if self.factory.require_overlap and not self.factory.overlapped.wait(timeout=5):
            raise AssertionError("Independent instance workers did not overlap")
        with self.factory.lock:
            self.factory.active_instances -= 1
