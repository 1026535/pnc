...
# Reviewed LV6 order facts of pet_workshop_lv6_20260917.png; card 3 is fully
# clipped (edge sliver only) so its requirements and rewards stay empty.
...
def _wire(ocr_service: Any) -> tuple[ObservationBuilder, NavigationPerception]:
...
class PetWorkshopBoardPublicationTests(unittest.TestCase):
    ...
    def test_lv6_board_publishes_reviewed_orders(self) -> None:
        """The LV6 board decodes its requirement pairs and partial edge card."""
        ...
        for name, observation in (
            ...
            with self.subTest(publisher=name):
                ...
                self.assertEqual((), orders[2].rewards)
                self.assertIsNone(orders[2].ready)
        self.assertEqual(builder_observation.workshop, navigation_observation.workshop)
...
class PetWorkshopModalPublicationTests(unittest.TestCase):
    ...
    def test_order_detail_surface_publishes_targets_and_rewards(self) -> None:
        ...
        # No submit control exists on the detail surface; ready stays false.
    ...
    def test_header_counters_read_on_real_ocr(self) -> None:
        ...
        expected: dict[str, dict[str, Any]] = {
            "pet_workshop.png": {
                "header": (8, 8, 166, 200),
                "rewards": (
                    ((WorkshopOrderRewardCategory.CHEST, 91),),
                    (
