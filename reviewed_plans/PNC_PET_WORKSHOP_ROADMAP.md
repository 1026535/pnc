...
| [PW03 — Effort, reservations and policy](pet_workshop_packets/PW03_EFFORT_RESERVATIONS_POLICY.md) | S1 | PW01 | Fixing findings | Shared recipe allocation/reconstruction is implemented at rebased candidate `0fbc61d`; 162 focused tests passed before a documentation-only base rebase. Final affected selection is running its required full fallback. One additional fresh-goal production revalidation gap remains; not accepted or merged. |
| [PW04 — One-step solver](pet_workshop_packets/PW04_ONE_STEP_SOLVER.md) | S2 | PW03 policy implementation | Fixing findings | Lead owns the stable solver checkout; Devin turn 003 has released its writer. Feed protection, extra-feed production, missing/expired generator reconstruction and finite exact-stock preservation are covered. Pure solver has no live dependency; acceptance awaits final correction and offline checks. |
