...
Live work is on hold: the configured castles on **3xx spies** are C1–C9, below Workshop’s C24 prerequisite, and the user is preparing main for a later lease. Wait for the user to release main before any live access; preserve the prior exclusion of Hopeful NPC. Do not substitute another instance. Resolve the replacement through configured identity, roles and the canonical lease once supplied. Use the active castle unless switching is explicitly authorized. Preserve existing resource restrictions and bind any consumptive canary to the exact target/action/budget authorized by the request or approved plan; the lease alone does not invent a spending amount. No live work is needed for PW01. The existing native Devin completion callback and monitor own automatic handoff delivery; keep worker run/turn, reviewed/tested revisions and evidence in packet records. Do not treat worker completion as acceptance.
...
| [PW02 — Recognition and controls](pet_workshop_packets/PW02_RECOGNITION_CONTROLS.md) | R1 | PW01 | Delegated | Accepted base `91afce6`; `cedar-stealer` turn 007 in the reused PW01 worktree. Offline recognition/evidence work; live acceptance awaits main availability. |
| [PW03 — Effort, reservations and policy](pet_workshop_packets/PW03_EFFORT_RESERVATIONS_POLICY.md) | S1 | PW01 | Fixing findings | Lead reviewed `a9f79ae`; 129 focused tests passed but concrete recipe, reservation and ranking counterexamples failed. Same worker `absorbing-vicuna` turn 002 fixes S1/S3–S6 from rebased `b868a5f` on current main `cf171fd`. Not accepted. |
| [PW04 — One-step solver](pet_workshop_packets/PW04_ONE_STEP_SOLVER.md) | S2 | PW03 policy implementation | Fixing findings | Combined solver correction batch also fixes S2: every proposed mutation must satisfy canonical validation. Final candidate offline evidence remains required; no live dependency for the pure solver. |
...
| [PW06 — Authority and journal](pet_workshop_packets/PW06_AUTHORITY_JOURNAL.md) | X1 authority/persistence | PW01 | Delegated | Accepted base `91afce6`; `fearless-sprite` turn 001 in `pet-workshop-authority`. No parser/solver/UI execution ownership. |
