...
- Under the canonical `serious_stuff` lease, the initial process catalog was empty.
- One launch of the exact `Nougat32` instance key was authorized. No ADB connection,
  game input, or other host action was performed.
- Six two-second catalog snapshots then showed only `Nougat32`, with one stable process
  ID and status port `5555` on every snapshot.
- The duplicate-port condition did not recur, and no transient duplicate that converged
  to a unique binding was observed. The historical collision therefore remains an
  unresolved host ambiguity; this evidence does not justify retry or convergence logic.
...
a newly launched target. The launch path should wait first for the exact process and
then for the exact runtime binding. Reuse the current `launch_poll_attempts` and
...
For this implementation, the transient cases above are intentionally not added: the
classification did not demonstrate a transient duplicate. The existing persistent
collision regression is strengthened with structured diagnostics and assertions that
resolution returns no instance and requests no launch.
...
This implementation skips the convergence helper because the completed classification
did not establish a transient race. The resolver keeps its immediate fail-closed
active-port rejection.
...
Implemented disposition: retain `ConfigurationError` and the existing target identity,
port, and `instance_keys` details; add `failure_phase="port_ambiguity"` and the
conflicting running records' display names. The focused regression asserts all of these
details and verifies that no `BlueStacksInstance` or launch result is produced. The
resolver message names each validated claimant, and the sanitized host CLI allowlists
`port_ambiguity` and projects only type-checked `instance_keys` and conflicting display
names.
...
No automatic recovery, new authorizer, host-state edit, or operator workflow change is
included in this narrow implementation. The existing lease-aware restart owner remains
the explicit recovery path if a persistent host collision is later confirmed.
...
- Entry point: build one connected runtime through the production resolver after
  offline checks.
...
The pre-implementation classification above was the only live action authorized for
this patch. No post-change host or live resolution action is included.
...
- The verified `origin/main` baseline is commit `5135dde` for hidden PowerShell monitor
  windows, and this feature worktree is based on that same commit. No unrelated local
  `main` changes are part of this implementation.

## Completion criteria

The live condition classification and one authorized `serious_stuff` production
resolver proof were completed before this diagnostics-only change, without game input.
The repair is complete when persistent collisions remain fail-closed with useful
diagnostics and the focused resolver checks pass. Because this change only adds error
diagnostics and regression assertions, no redundant post-change live run is required.
When a transient race is demonstrated in a future classification, the bounded
convergence path must be implemented and validated separately. If the host retains a
persistent collision, code may be complete but live use remains blocked until the exact
conflicting display-name bundle is explicitly restarted.
