...
Status (2026-09-12): implementation in progress. A–E have substantial implementation and historical validation; replacement-navigation integration and the remaining F family evidence are incomplete. Resume with the integration slice below. This document does not authorize game mutations.
...
The working branch is `codex/non-yolo-recognition`, rebased through mainline `aabe85b007a74150303ac9ec43e0bf6fc1fc7e37`, with reviewed feature tip `02f07ac91d7162de9550aaef2e72f8ae3288392d`. A fresh fetch also found `d99fdb1` and `0542c7b` on `origin/main`: offline test modularization and timing evidence. Those two commits are not in this checkout; the commands below use this checkout's flat test modules and must be mapped to the modular owners at the next synchronization. Do not overwrite unrelated main-worktree work or restart implementation from the historical baseline above.
...
| B0/B: observation and actions | Frame/input provenance, global guards and canonical immutable `ScreenDecision` in `ObservationBuilder` | Remove rebase-era synthetic provenance and contradictory-decision shortcuts; bring replacement `NavigationPerception` under the same safety contracts |
...
| D: OCR | Frame-local immutable context, cached reads, compiled field regions and diagnostics | Replacement navigation still reads guards separately and constructs content context without capture identity; share genuine frame context and preserve requested-region validation |
