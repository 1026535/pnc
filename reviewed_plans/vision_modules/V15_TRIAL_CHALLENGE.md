...
- `vision/trial_challenge.py` (`TrialContentProducer`) reads six fixed measured card slots only under `trial_challenge_live`. Identity comes from each card's own bounded title read — never slot order. Progress, castle requirement, countdown, weekday, lock glyph, reward chest, Trial-chip presence and Stats-chip presence are independent facts: a countdown can coexist with a castle lock (Rune), a chest never implies completion, and an occluded weekday stays unknown. The toolbar counter publishes once at screen level; the then-accepted 1.2.3 OCR backend could not read the reference lone `0` glyph. The shared [OCR modernization plan](../PNC_OCR_TEXT_LOCALIZATION_MODERNIZATION_PLAN.md) must reproduce that case on the now-accepted V13 backend before claiming it remains a failure.
- Only an unlocked Gear card's measured Stats chip is actionable (row COMPLETE with chip bounds/point); every other row is NO_ACTION regardless of visible chips, and duplicate resolved categories mark rows AMBIGUOUS with no action. Trial/Rank/Exchange/Mall/Progress/Total Rank controls stay observation-only — content OCR never grants action authority.
...
Gear Stats is the only proved detail family. Other categories publish complete card facts but remain unsupported for inspection — they are not force-fit to the Gear detail, and any deeper route for them needs its own evidence. Trial entry, battle, stamina/resource use, Exchange purchases and claims are outside this packet. V15 is accepted for this stated coverage after architectural/caller review, the three lead corrections below and the production live route.


## Acceptance evidence, September 16

The lead reviewed canonical domain ownership, both publication paths and their provenance/merge handling, the demand-driven producer, measured controls, NavigationCore and WorkflowContext integration. Three final corrections preserve Trial summary/detail through guarded observation merging, suppress Stats actions on locked Gear cards, and parse a whole integer percentage without turning a fractional token into a wrong integer. Unsupported numeric forms retain their literal text and an unknown numeric value.

- Worker candidate full fallback: **2,347 passed, 7 skipped** (2,354 total), machine results `.test-impact/results_v15_candidate.json` in `vision-v01-foundation`.
- Lead corrected candidate: **31 passed** in 33.917s across the Trial unit, real-OCR both-publisher integration and Trial navigation tests; `.local-data/devin-v15/lead-correction-tests.log`.
- Final production live route **passed** on the active castle of `mega_old_acc`: Home → Tower → fresh six-card Trial content → `WorkflowContext.open_trial_stats(GEAR)` → nine Applicable Stats rows → Trial → Home. Runtime `20260916T102239Z_f31a49a3`; evidence `.local-data/devin-v15/live_candidate_content/` in the same checkout. Source frame `0029` at 10:23:46Z, confirmed Stats frame `0032` at 10:24:06Z and final Home frame `0041` at 10:24:18Z are CLEAR. The lead visually checked the Stats table and final Home. No spending actions; the lease released and the pre-existing instance was preserved.
- An earlier harness attempt stopped on the identity-only navigation result before Stats input. The harness was corrected to request fresh content; no production retry or parser workaround was added for that harness error. Its failed evidence remains in `live_candidate/`.
