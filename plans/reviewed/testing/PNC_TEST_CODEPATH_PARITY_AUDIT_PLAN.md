...
- `plans/reviewed/core/PNC_OPERATION_PREFLIGHT_WORKFLOW_VALIDATION_PLAN.md` defines the separate workflow ownership model.
...
| `tests/test_live_chat_workflow_smoke.py` | Opt-in live helper/workflow smoke | Connected observation/session plus connected runtime `ScreenFlowPlanner` and `ObservedActionExecutor` used directly by the reusable chat workflow | Do not treat as send-chat task proof; add script-run smoke if task/runner parity is needed |
...
Use `plans/reviewed/core/PNC_OPERATION_PREFLIGHT_WORKFLOW_VALIDATION_PLAN.md` when deciding whether a test should be runner-level or task-level.
