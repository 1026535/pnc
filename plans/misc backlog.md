...
**Validation status:** Stage 1 of item 3 is implemented and its focused unit module passed. Logging integration and caller migration remain pending; their proposed checks have not been run.
...
**Stage 1 status:** Core implemented; integration pending. `py -3.13 -m unittest tests.unit.core.infra.diagnostics.test_async_diagnostic_writer` passed.
