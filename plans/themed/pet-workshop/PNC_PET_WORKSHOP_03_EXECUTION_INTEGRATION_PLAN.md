...
Dependencies: [Plan 01 shared contract/recognition](PNC_PET_WORKSHOP_01_RECOGNITION_STATE_PLAN.md), [Plan 02 solver/policy](PNC_PET_WORKSHOP_02_SOLVER_POLICY_PLAN.md), and the current canonical runtime. The [Feature 06 native-RGBA identity regression](../account-runtime/PNC_CORE_REMAINING_06_CASTLE_NAVIGATION_PLAN.md#september-16-regression-native-screenshot-drops-the-selected-hopium-row) must be closed before unattended target selection is accepted. Do not duplicate its fix here or replace failed preflight with a configured expected name or agent visual assertion.
...
| Navigation | `NavigationCore.open_building` and `reviewed_navigation_edges`: Home → Illusory Beast Manor → Pet Workshop, information panels and observed returns. PW07 owns the [Manor target and route qualification](packets/PW07_WORKFLOW_EXECUTION.md#home-entry-ownership-after-the-v44-review--2026-09-21); V44 owns shared camera/slot/scan contracts. Add actual Manor identity to the canonical Home catalog, consuming accepted acquisition behavior; do not use Trap Workshop or create a second city navigator. |
...
3. Enter through qualified canonical Home-building acquisition, then reviewed Manor → Workshop controls. Follow PW07's V44 dependency boundary; an unqualified acquisition path returns unavailable rather than a guessed action. Construction of the connected runtime must not navigate. No saved coordinate tap harness is promoted into the product.
...
| Packet | Deliverable |
| --- | --- |
| [PW06](packets/PW06_AUTHORITY_JOURNAL.md) | Canonical feature authority/budget, invocation factory and journal migration/reconciliation lifecycle. |
| [PW07](packets/PW07_WORKFLOW_EXECUTION.md) | Observed action session/receipts, Workshop lifecycle, routes/survey and common continuation/results. |
| [PW08](packets/PW08_INDEPENDENT_RUNS.md) | Independent CLI/API and authored-task binding. |
| [PW09](packets/PW09_DAILY_MAINTENANCE.md) | Daily Maintenance adapter and invocation/isolation behavior. |
| [PW10](packets/PW10_LIVE_QUALIFICATION_INTEGRATION.md) | Lead combined review, bounded live qualification and final integration. |
...
Dependency sequencing and worker instructions are maintained once in the [roadmap](PNC_PET_WORKSHOP_ROADMAP.md). The live proof obligations are maintained in [PW10](packets/PW10_LIVE_QUALIFICATION_INTEGRATION.md).
