...
| [PW02 — Recognition and controls](packets/PW02_RECOGNITION_CONTROLS.md) | R1 | PW01 | Fixing findings offline | Final candidate `4b3926a`: 2,831 tests passed / 7 environmental skips. Item/order detail and help ownership passed live; storage reaches the board parser and crashes on OCR capacity=0. The reported same-frame failure is an invalid encoded/decoded hash comparison, independently adjudicated. Main released at Home; live worker's repeated phases exceeded the short-window instruction and are not accepted as precedent. Saved-frame curation and lead correction continue offline. |
