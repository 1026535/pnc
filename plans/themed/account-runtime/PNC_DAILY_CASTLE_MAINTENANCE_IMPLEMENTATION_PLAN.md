...
Current A/B continuation: [shared workload and ownership](../operations/PNC_AB_COORDINATED_CONTINUATION.md).
...
| NPC 2 Arena variability | `artifact_answered`, not a promotion gate | The 2026-09-01 guarded entry typed Intro and Formation and returned without Save/Challenge/attempt. The optional seven-entry audit may continue for operational knowledge, but the user-approved promotion rule is one positive live canary per distinct Arena behavior variant. See `plans/reviewed/gameplay/PNC_HERO_ARENA_SEVEN_ENTRY_AUDIT.md`. |
