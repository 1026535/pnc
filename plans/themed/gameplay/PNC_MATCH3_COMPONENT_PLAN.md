...
**Combat opportunity checkpoint, 2026-09-23:** the reviewed pure selector ranks supported board actions by observed front-column exposure, imminent enemy countdown and same-color hero MP opportunities, using explicit `column_front` routing from the packaged non-PVP client. It preserves the canonical rule engine's unsupported/no-action guards and stable baseline tie-break, and states that the ranking is uncalibrated. Seven combat scenarios and all 132 domain tests passed. The affected runner selected the full portable suite because this adds a shared domain module: 3,006 passed and seven skipped across 344 modules on `a9c665b` (source fingerprint `0d1f4dfd577a55b9ab6cf0287602675b90ff470e9a798a46d3da9d17d4aa4403`; `.test-impact/m1-combat-opportunity-results.json`). Rebasing over nonoverlapping policy/test-harness changes left all five slice files byte-identical. This accepts only the pure opportunity slice; current battle reports and live board observations are still missing, so it is not a qualified win/survival estimate, calibrated multi-turn policy or operational mode.
