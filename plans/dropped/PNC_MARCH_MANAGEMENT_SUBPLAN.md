...
- [PNC_BUILDING_ACTIONS_SUBPLAN.md](abandoned/PNC_BUILDING_ACTIONS_SUBPLAN.md), which owns buildings such as `alliance_hall`, `market`, `hall_of_war`, and `pit`, plus the existence of their building-owned entry screens,
- [PNC_SCREEN_FLOW_ARCHITECTURE.md](PNC_SCREEN_FLOW_ARCHITECTURE.md), which owns reusable navigation patterns,
- [PNC_TASK_SUBPLAN.md](PNC_TASK_SUBPLAN.md), which owns broader feature-task planning,
- [PNC_AUTOMATION_IMPLEMENTATION.md](abandoned/PNC_AUTOMATION_IMPLEMENTATION.md), which owns the primary platform architecture.
...
From the current user guidance and screenshots already referenced by [PNC_BUILDING_ACTIONS_SUBPLAN.md](abandoned/PNC_BUILDING_ACTIONS_SUBPLAN.md):
...
1. [PNC_BUILDING_ACTIONS_SUBPLAN.md](abandoned/PNC_BUILDING_ACTIONS_SUBPLAN.md) keeps the high-level facts that building-owned entry flows such as `alliance_hall -> alliance_member_reinforce` and `market -> alliance_member_transport` exist.
