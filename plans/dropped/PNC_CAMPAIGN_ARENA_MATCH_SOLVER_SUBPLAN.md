...
- [PNC_BUILDING_ACTIONS_SUBPLAN.md](abandoned/PNC_BUILDING_ACTIONS_SUBPLAN.md), which owns `campaign` and `arena` as buildings and the existence of the `campaign_map` and `versus_center` linked screens,
...
1. [PNC_BUILDING_ACTIONS_SUBPLAN.md](abandoned/PNC_BUILDING_ACTIONS_SUBPLAN.md) keeps the high-level facts that `campaign -> campaign_map` and `arena -> versus_center` exist.
