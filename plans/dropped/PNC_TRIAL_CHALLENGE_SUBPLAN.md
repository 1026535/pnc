...
> **Status: DROPPED — historical plan.** Current vision/menu work is [V15 Trial Challenge](../themed/vision/modules/V15_TRIAL_CHALLENGE.md) under the [modular plan](../themed/vision/PNC_VISION_MODULAR_PLAN.md). The historical route inventory below requires current evidence; its broader action/framework scope is not reactivated.
...
- [PNC_BUILDING_ACTIONS_SUBPLAN.md](abandoned/PNC_BUILDING_ACTIONS_SUBPLAN.md), which owns `tower_of_trial` as a building and the existence of the `trial_challenge` linked screen,
- [PNC_SCREEN_FLOW_ARCHITECTURE.md](PNC_SCREEN_FLOW_ARCHITECTURE.md), which owns reusable screen-navigation patterns,
...
1. [PNC_BUILDING_ACTIONS_SUBPLAN.md](abandoned/PNC_BUILDING_ACTIONS_SUBPLAN.md) keeps the high-level fact that `tower_of_trial -> trial_challenge` exists.
