...
- Treat an unrelated popup as a recoverable test interruption. If canonical popup recovery fails, capture a fresh screenshot, identify an unambiguous dismissal control on that frame, and use a manual on-screen tap to close it when the action stays within the assigned lease, castle, role, and spending authority. Reobserve the screen and resume the batch when the required preconditions hold. Do not infer a close control, use Android Back as a guess, dismiss a task-owned dialog, or take an unauthorized claim, purchase, switch, or other mutation to clear a popup.
- For an unrelated castle-identity or BlueStacks instance-management failure, the assigned tester or authorized live-test worker should make the smallest evidence-backed, bounded correction through existing identity, status, or readiness entry points, then revalidate identity and readiness and resume the planned checks. Host-management mutation is allowed only when already in the assignment's scope and permitted by role, idle-lease, and `read_only` controls. Keep live-test acceptance as the priority: do not turn the batch into an unrelated code repair or open-ended host investigation. If no permitted quick correction establishes the preconditions, leave dependent cases pending, continue independent checks, and assign the underlying defect separately. Never bypass a foreign reservation or unverifiable identity.
- For a named, high-impact critical-path blocker with dependent work waiting, the coordinator may use the bounded correction checkpoint in `.agents/skills/test-bluestacks-live/SKILL.md` only when one short lead-owned feedback cycle is likely to reduce blocked time and the current request or approved plan authorizes both lead source edits and lead live execution. A Devin-only assignment remains worker-owned until its handback is terminal and its processes and live resources are released. Routine failures remain in the assigned batch. Otherwise leave affected cases pending and record the next owner and trigger in the batch and, when the work belongs to an epic, its coordinator ledger.
- Record every castle selection/identity, popup, or BlueStacks instance-management failure, including preflight and recovered incidents, through [.agents/skills/test-bluestacks-live/references/failure-reporting.md](.agents/skills/test-bluestacks-live/references/failure-reporting.md). Publish the incident in the shared weekly report collection and link its stable ID and report path from the live-test batch and delegated evidence manifest. Classify unrelated popup, navigation, lease, castle-identity, or target-setup failures at the boundary where they occurred. Finish other safe independent checks and continue offline development. Keep affected live cases pending until their own postconditions are observed; an environmental failure is neither a feature failure nor passing proof. Never bypass lease or identity authority to get a result.
...
- Use `devin-implement` for requested Devin implementation delegation and `devin-game-knowledge` for bounded consultation. When the user delegates live testing to Devin, use `devin-live-test` for the assigned execution and routine live follow-up. For a named critical-path blocker with dependent work waiting, the lead may perform the bounded correction checkpoint in `test-bluestacks-live` only when one short lead-owned feedback cycle is likely to reduce blocked time, the current task authorizes lead edits and live execution, and the Devin assignment has returned its resources; an explicit Devin-only instruction remains binding. The lead owns acceptance, and a checkpoint proves only its named cases. `test-bluestacks-live` remains the canonical live policy.
