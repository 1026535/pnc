...
def _rescale_popup_overlay(
    ...
    *,
    ...
    if overlay is None or overlay.image_size == target_size:
        return overlay
    source_width, source_height = overlay.image_size
    target_width, target_height = target_size

    def scale_bounds(bounds: Bounds) -> Bounds:
    ...
    candidates = tuple(
        PopupDismissCandidate(
            ...
            bounds=scale_bounds(candidate.bounds),
            action_point=(
                round(candidate.action_point[0] * target_width / source_width),
                round(candidate.action_point[1] * target_height / source_height),
            ),
    ...
    return PopupOverlayObservation(
        ...
        modal_bounds=None if overlay.modal_bounds is None else scale_bounds(overlay.modal_bounds),
