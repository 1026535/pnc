    ...
    if parsed.command == "construct":
        ...
        with application.reserve_accounts((parsed.account,)):
            ...
            step_result = application.run_task(
                ...
                params={
                    "building": parsed.building,
                    **({"operation_id": parsed.operation_id} if parsed.operation_id is not None else {}),
                },
                required_role=required_role,
            )
...
def _add_building_upgrade_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--operation-id",
        help="Caller-owned durable operation id reused when reconciling an interrupted upgrade.",
    )
...
def _add_building_construction_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--operation-id",
        help="Caller-owned durable operation id reused when reconciling an interrupted construction.",
    )
