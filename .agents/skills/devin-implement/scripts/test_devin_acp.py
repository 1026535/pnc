...
class AcpTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        connection.context = {"used_tokens": None, "peak_used_tokens": None,
                              "window_tokens": None, "compactions_completed": 0}
    ...
    def test_context_metrics_exclude_replay_side_and_other_sessions(self):
        ...
        def receive(method="session/update", session="test", chain="main", **fields):
            self.connection.events.put({"method": method, "params": {
                "sessionId": session, "_meta": {"cognition.ai/chain": chain}, **fields}})
            self.connection.receive()

        usage = {"sessionUpdate": "usage_update", "used": 130770, "size": 262000,
                 "_meta": {"private": "Do not retain usage history or content"}}
        self.connection.live = False
        receive(update=usage)
        self.connection.live = True
        receive(update=usage, session="other")
        receive(update=usage, chain="side")
        receive("_cognition.ai/compaction", chain="side", status="completed")
        self.assertFalse((self.turn / "context.json").exists())
        receive(update=usage)
        receive("_cognition.ai/compaction", status="started")
        receive("_cognition.ai/compaction", status="completed", summary="PRIVATE SUMMARY")
        receive(update={**usage, "used": 36425})
        receive(update={**usage, "used": None})
        self.connection.responses[99] = {"error": {"code": -32013, "message": "internal"}}
        with self.assertRaisesRegex(RuntimeError, "-32013"):
            self.connection.result(99)
        self.assertEqual(worker.read_json(self.turn / "context.json"), {
