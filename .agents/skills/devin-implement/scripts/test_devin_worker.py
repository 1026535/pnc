...
class MonitorTests(unittest.TestCase):
    ...
    def test_status_excludes_content_and_bounds_diagnostics(self):
        ...
        context = {"used_tokens": 36425, "peak_used_tokens": 130770,
                   "window_tokens": 262000, "compactions_completed": 1}
        worker.write_json(self.root / "context.json", context)
...
''', encoding="utf-8")
    ...
    def test_fresh_and_resume_preserve_state(self):
        ...
        self.assertEqual(state["compaction_threshold_tokens"], 100_000)
        self.assertEqual(state["context"]["compactions_completed"], 1)
        self.assertEqual(state["permission_mode"], "dangerous")
        self.assertFalse(state["respect_workspace_trust"])
