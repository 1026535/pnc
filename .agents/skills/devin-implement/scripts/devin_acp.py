...
class Connection:
    ...
    def __init__(self, command, cwd, turn_dir, permission_mode="normal"):
        ...
        self.main_tools = set()
        self.context = {"used_tokens": None, "peak_used_tokens": None,
    ...
    def receive(self, timeout=0.1):
        ...
        if message["method"] == "_cognition.ai/compaction":
            if params.get("status") == "completed":
                self.context["compactions_completed"] += 1
                write_json(self.directory / "context.json", self.context)
        elif kind == "usage_update":
            # These are context occupancy snapshots, not additive billed usage.
            used, size = update.get("used"), update.get("size")
            if type(used) is int and used >= 0 and type(size) is int and size > 0:
                self.context.update(used_tokens=used, window_tokens=size,
                                    peak_used_tokens=max(self.context["peak_used_tokens"] or 0, used))
                write_json(self.directory / "context.json", self.context)
