...
The catalog preflight allows three attempts total (the initial request and two retries), waiting two and four seconds after the observed `GetCliModelConfigs` connection/send error. Each failed attempt retains its stderr in the turn directory. Authentication, authorization, quota, malformed catalog, and missing-model errors are not retried. This policy never replays an ACP prompt or game action. Exhausted preflight records a failed result, releases writer ownership, and uses the normal completion callback; after resolving the failure, `--resume` advances to a new turn without deleting evidence. A fresh run that failed before creating a Devin session starts a new session on that explicit resume.
