...
- [failure reporting](../test-bluestacks-live/references/failure-reporting.md) — mandatory durable reports for popup, instance-management, and castle selection/identity failures, including preflight and recovered incidents. Resolve the brief's shared report root before target preflight and publish at the first safe checkpoint, not only at final handback.
- [popup recovery](../devin-live-test/references/popup-recovery.md) — the worker playbook for unrelated blocking popups: screenshot interpretation against the measured popup playbook, canonical-then-manual dismissal, reobserve-and-resume, and incident publication.
...
- On failure, inspect the screenshots, OCR, observations, and logs the runtime wrote under its configured artifact root. For an unrelated popup, follow the [popup recovery playbook](../devin-live-test/references/popup-recovery.md): identify the family on a fresh frame, try canonical bounded recovery first, and if it fails tap the playbook's measured dismissal control under the assigned lease and authority; reobserve before continuing. Do not guess controls or perform an unauthorized mutation. Record the popup failure and recovery even if the feature check later passes; do not build generalized recovery.
- For an unrelated castle-identity or BlueStacks instance-management failure, make a quick evidence-backed correction through existing identity, status, or readiness entry points, then revalidate identity and readiness and resume the checks. Host-management mutation requires explicit assignment scope and the role, idle-lease, and `read_only` boundaries. Record the incident even if recovered. Do not edit code or config or spend the assignment on unrelated host diagnosis; leave dependent checks `not_run` and identify a separate follow-up when no permitted bounded correction works.
...
- Never modify local config to make a check pass, and never edit source, tests, fixtures, plans, tracked documentation, or Git state; no commits or pull requests. Ignored runtime evidence, required incident folders under the brief's shared report root, and the result package are permitted outputs. Leave shared collection indexes to the collection owner or weekly reporting job.
...
3. Write one `evidence.json` in `DEVIN_IMPLEMENT_TURN_DIR` following the curated evidence contract in the live-test brief. Verify every assigned check has a verdict with non-passing results explained, every interruption links its incident ID and published report path (or explicitly pending local report), every cited artifact exists and belongs to this run, the candidate matches, and cleanup plus every lease release are explicit.
