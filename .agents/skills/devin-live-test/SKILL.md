...
- An active long reservation authorizes its owner to start a stopped configured instance inside that reservation to restore the assigned work, provided the configured role permits instance launch. Include this bounded recovery authority in the brief. Devin must renew and validate the matching receipt, acquire the canonical task lease, confirm the instance is stopped, and use the existing resolver. Leave a recovered target running at handback; closing it requires separate explicit authorization. This does not authorize stopping or restarting a running instance, launching outside the reservation, exceeding role or `read_only` limits, switching accounts or castles, or spending resources.
- Do not default a reserved target to `allow_launch=False` or prohibit this recovery in its brief. Narrow the recovery authority only when the current request or approved plan explicitly excludes starting that target, or its configured role does not permit launch.
...
Write the assignment with [live-test-brief.md](references/live-test-brief.md). Missing live authority, an ambiguous mutation boundary, or an unspecified required spending budget blocks launch; a valid active reservation supplies bounded start authority only for its stopped configured targets as described above. Ordinary execution choices do not.
