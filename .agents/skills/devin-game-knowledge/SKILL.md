...
Name the smallest relevant source files and saved evidence. Ask for targeted reads and a compact memo with artifact paths, without importing full histories, exports, or artifact trees. Keep live execution in [devin-live-test](../devin-live-test/SKILL.md) when the user has delegated live testing to Devin; a consultation supplies findings and the next observation, not a competing live session. This one-shot consultation launcher does not inherit the implementation launcher's compaction overlay.
...
Each run writes a scoped `--config` that pre-approves only path-scoped reads (source, tests, docs, plans, skills, and `.local-data/` evidence — never `Read(**)`) and read-only inspection commands (`git log`/`diff`/`show`-class plumbing, `rg`/`grep`-class searches). It denies mutating Git, destructive shell, install, `adb`, and every interpreter or shell (`python`, `node`, `bash`, `cmd`, `powershell`, …), and it explicitly denies local configuration, account data, and credential paths (`config/**`, `**/accounts*.y*ml`, `.env`, key/cert files). This keeps a headless run from stalling on a confirmation prompt without widening the write or secret surface — a broad interpreter grant would escape the read-only boundary regardless of the deny list.
