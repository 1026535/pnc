...
| Authored scripts | `TaskId.ENSURE_GAME_RUNNING`, `TaskId.POPUP_RECOVERY`, `TaskId.COLLECT_KINGDOM_CHAT`, `TaskId.SEND_WORLD_CHAT_MESSAGE`, `TaskId.COLLECT_MAIL`, `TaskId.OPEN_BUILDING`, and `TaskId.REFRESH_CASTLE_ROSTER` use the typed core dispatcher; other registered `TaskId`/YAML steps remain on legacy dispatch. |
...
| Chat and mail sending | Authored `SEND_WORLD_CHAT_MESSAGE` uses `SendKingdomChatWorkflow` and the shared typed chat-send operation. Alliance and mail sending remain legacy; Alliance is blocked before task preflight or input while its binding is unported. Live messages need an authorized destination and exact content. |
...
`SendKingdomChatWorkflow` binds authored `SEND_WORLD_CHAT_MESSAGE` to a Home-to-Home `NONSPENDING_STATE_CHANGE` workflow. It uses the shared `ChatMessageTaskParams` parser, exact active-castle preflight, and `WorkflowContext.send_chat_message`; it has no archive dependency or legacy fallback. The context rejects sending from a read-only workflow.
...
The normal input selector retains its canonical OCR region in the registry. Its tap authority comes exclusively from the Chat visual profile's current-frame template match. The focused-empty placeholder is a non-clickable evidence label; the focused gold return is the submit action. The unverified blue Send control remains unsupported. A receipt requires an empty draft and a strictly increased count of visible player rows matching the active castle and message. Sender matching strips one leading alliance tag and uses the shared castle matcher; transcript matching uses canonical whitespace normalization while preserving punctuation and content. This is observed receipt evidence, not a server acknowledgement or an exactly-once guarantee. If an old identical row leaves the viewport as a new one arrives, the unchanged count deliberately remains unconfirmed.

For another chat binding, reuse this operation and the canonical parser. Do not add another typing loop, approval flag, send retry, or message normalizer. Capture the exact authorized message budget and inspect the posted receipt and final Home evidence before declaring the live proof complete.
